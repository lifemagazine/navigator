...
public class AircraftTransport implements ITransport, Serializable {
	
	private static final long serialVersionUID = -8798205991877440851L; 
	
	private Logger LOG = LoggerFactory.getLogger(AircraftTransport.class);
	...
	public AircraftTransport(String key, String name, IPort sourcePort, IPort destinationPort, 
		...
		try {
			this.cost = Math.round(NavigatorConfig.get().getCost(TransEnum.AIRCRAFT) * this.distance);
//			LOG.info(this.distance + ", " + this.cost + ", NavigatorConfig.get().getCost(TransEnum.AIRCRAFT) = " + NavigatorConfig.get().getCost(TransEnum.AIRCRAFT));
		...
		this.transType = TransEnum.AIRCRAFT;
		this.status = status;
	}
