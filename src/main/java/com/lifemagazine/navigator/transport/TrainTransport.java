...
public class TrainTransport implements ITransport, Serializable {
	
	private static final long serialVersionUID = -8798205991877440851L;
	
	private Logger LOG = LoggerFactory.getLogger(TrainTransport.class);
	...
	public TrainTransport(String key, String name, IPort sourcePort, IPort destinationPort, 
		...
		try {
			this.cost = Math.round(NavigatorConfig.get().getCost(TransEnum.TRAIN) * this.distance);
		...
		this.transType = TransEnum.TRAIN;
		this.status = status;
	}
