...
public class VesselTransport implements ITransport, Serializable {
	
	private static final long serialVersionUID = -8798205891877440851L; 
	
	private Logger LOG = LoggerFactory.getLogger(VesselTransport.class);
	...
	public VesselTransport(String key, String name, IPort sourcePort, IPort destinationPort, 
		...
		try {
			this.cost = Math.round(NavigatorConfig.get().getCost(TransEnum.VESSEL) * this.distance);
		...
		this.transType = TransEnum.VESSEL;
		this.status = status;
	}
